				   ...
				   uint8_t * ctr, size_t ctrLen)
{
	...
	auto seal_status = sgx_unseal_data((sgx_sealed_data_t *)sealKey, NULL, NULL, unsealedKey, &keyLen);
	assert(seal_status == SGX_SUCCESS);

	ocall_encrypt_file(path, ctr, ctrLen);
				   ...
				   uint8_t * ctr, size_t ctrLen)
{
	...
	ocall_decrypt_file(path, ctr, ctrLen);
	memset(unsealedKey, 0, SGX_AESCTR_KEY_SIZE);
}

void ecall_encrypt_aes_ctr(char * plainMessage, size_t lenPlain,
...
void ecall_decrypt_aes_ctr(uint8_t * cryptMessage, size_t lenCrypt,
						   uint8_t * counterBlock, size_t lenCounter,
